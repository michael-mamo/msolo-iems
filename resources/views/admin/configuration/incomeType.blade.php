            ...
            <div class="card">
              ...
              <div class="card-body">
                  ...
                  <div class="modal-dialog">
                    <div class="modal-content">
                      <div class="modal-header">
                        <h4 class="modal-title">Income Type Registration</h4>
                        ...
                        <form action="{{route('incomeType.add')}}" method="POST">
                          ...
                          <div class="card-body">
                            <div class="form-group">
                              ...
                              <input name ="name" type="text" class="form-control" id="incomeType"
                ...
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                      @foreach($allData as $key=>$incomeType)
                    <tr>
                      <td>{{$key+1}}</td>
                      <td>{{$incomeType->name}}</td>
                      <td>{{$incomeType->description}}</td>
                      <td>@if($incomeType->isactive == '1')
                          Yes
                          @else
                          No
                          @endif
                      ...
                      <td class="text-center">
                        <div class="btn-group bg-success">
                          <button type="button" class="btn btn-sm btn-secondary float-left" data-toggle="modal"
                            data-target="#modal-editRole{{$incomeType->id}}">
                            <i class="fas fa-edit"></i></span>
                          </button>
                          <a href="{{route('incomeType.delete', $incomeType->id)}}" id="delete" type="button" class="btn btn-sm btn-danger">
                            <i class="fas fa-trash"></i>
                          </a>
                        </div>
                      </td>
                    </tr>
                    <div class="modal fade" id="modal-editRole{{$incomeType->id}}">
                        <div class="modal-dialog">
                            <div class="modal-content">
                            <div class="modal-header">
                                <h4 class="modal-title">Edit {{$incomeType->name}}</h4>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                            <div class="modal-body">
                                <form action = "{{route('incomeType.edit', $incomeType->id)}}" method="POST">
                                    @csrf
                                <div class="card-body">
                                    <div class="form-group">
                                    <label>Description</label>
                                    <textarea name ="description" class="form-control" rows="4"
                                        placeholder="Type some description about the role">{{$incomeType->description}}</textarea>
                                    </div>
                                </div>
                                <div class="form-check">
                                    <input name="isActive" type="checkbox" class="form-check-input" id="editIsActive" {{($incomeType->isactive == '1'?"checked":"")}}>
